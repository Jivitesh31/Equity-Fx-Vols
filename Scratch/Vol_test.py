...
# V0 = 4437.86;  # EUROSTOXX50 index at '2016-01-15'
# qd = '2023-08-15';

cc = 0.015;  # example cost of carry

# excel file read
# xl = pd.ExcelFile("SPX.xlsx");
...
euro['TTM'] = ((euro['EXP_DATE'].apply(pd.to_datetime)-pd.to_datetime(qd))/np.timedelta64(1, 'D')).astype(int)/365
# for row in range(len(euro)):
#     pass
    # euro.at(row, 'TTM') = 1.0 * (time_tango(euro.loc[row]['EXP_DATE']) - time_tango(qd)).days / convDates
...
for i in range(1, len(newTtm) - 1):
    for j in range(1, len(newStk) - 1):
        ...
        dx[i, j] = (-1.0 / (1* (h[j - 1] + h[j])) * y2[i, j - 1]
                    + 0
                    + 1.0 / (1.0 * (h[j - 1] + h[j])) * y2[i, j + 1]);
